...
router.get('/:id', (req, res) => {
  ...
  const orders = prepare('SELECT * FROM orders WHERE customer_id = ? ORDER BY created_at DESC LIMIT 20').all(req.params.id);
  const subscription = prepare('SELECT * FROM subscriptions WHERE customer_id = ? AND status = ?').get(req.params.id, 'active');
...
router.post('/', (req, res) => {
  ...
  const existing = prepare('SELECT * FROM customers WHERE phone = ?').get(phone);
  ...
  if (referred_by) {
    prepare('INSERT INTO referrals (referrer_id, referred_id) VALUES (?, ?)').run(referred_by, result.lastInsertRowid);
    prepare(`INSERT INTO loyalty_log (customer_id, action, description) VALUES (?, 'referral', ?)`).run(referred_by, 'Nouveau filleul enregistré');
  }
...
router.put('/:id', (req, res) => {
  const existing = prepare('SELECT * FROM customers WHERE id = ?').get(req.params.id);
  ...
  prepare(`UPDATE customers SET name=?, phone=?, city=?, neighborhood=?, is_vip=?, purchase_count=? WHERE id=?`).run(
...
router.post('/:id/increment-purchase', (req, res) => {
  const customer = prepare('SELECT * FROM customers WHERE id = ?').get(req.params.id);
  if (!customer) return res.status(404).json({ error: 'Client introuvable' });
  ...
  prepare('UPDATE customers SET purchase_count = ?, is_vip = ? WHERE id = ?').run(newCount, isVip, req.params.id);

  if (newCount === 3) prepare(`INSERT INTO loyalty_log (customer_id, action, description) VALUES (?, 'vip', ?)`).run(req.params.id, 'Badge VIP obtenu (-10% permanent)');
  if (newCount === 5) prepare(`INSERT INTO loyalty_log (customer_id, action, description) VALUES (?, 'reward', ?)`).run(req.params.id, '5ème achat : 250ml offert');
