...
router.get('/', async (req, res) => {
  const rows = await prepare('SELECT * FROM products WHERE active = 1 ORDER BY sort_order ASC').all();
  res.json(rows);
});
...
router.put('/:id', async (req, res) => {
  const existing = await prepare('SELECT * FROM products WHERE id = ?').get(req.params.id);
  ...
  await prepare(`
    ...
    WHERE id=?
  `).run(
    name || existing.name, size_ml || existing.size_ml,
